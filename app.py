...
st.set_page_config(
    page_title="Wingo Matrix Omni-Engine v12.0 Apex",
    page_icon="👑",
    layout="wide",
)
...
with c1:
  st.markdown(
      "<div style='background-color:#143d22; padding:12px; border-left:5px"
      " solid #2ecc71; border-radius:5px; font-weight:bold;"
      " color:#f8fafc;'>🤖 10,000,000 MEGA DATA BASE: ONLINE<br><small"
      " style='color:#a8e6cf;'>(FAST FLASH CACHE)</small></div>",
      unsafe_allow_html=True,
  )
with c2:
  st.markdown(
      "<div style='background-color:#1c3144; padding:12px; border-left:5px"
      " solid #3498db; border-radius:5px; font-weight:bold;"
      " color:#f8fafc;'>⚡ HIGH-QUALITY AI CORE SERVER v12.0:<br><small"
      " style='color:#7efff5;'>APEX ULTRA RUNNING</small></div>",
      unsafe_allow_html=True,
  )
with c3:
  st.markdown(
      "<div style='background-color:#3d3414; padding:12px; border-left:5px"
      " solid #f1c40f; border-radius:5px; font-weight:bold;"
      " color:#f8fafc;'>🔥 AI GLOBAL MOVEMENT DETECTOR & 5.0 BILLION QUANTUM"
      " CLOUD: LOCKED</div>",
      unsafe_allow_html=True,
  )
  ...
  with b1:
    if st.button("🚀 ➕ Add Data to History", use_container_width=True):
      ...
      if st.session_state.pending_prediction is not None:
        bs_wl = (
            "W"
            if st.session_state.pending_prediction == actual_bs
            else "L"
        )
      else:
        bs_wl = "-"
      ...
      if st.session_state.pending_color_prediction is not None:
        rg_wl = (
            "W"
            if st.session_state.pending_color_prediction == actual_color
            else "L"
        )
      else:
        rg_wl = "-"
...
with col2:
  ...
  if st.session_state.result_history and st.session_state.period_history:
    ...
    st.markdown(
        f"⏳ **Last 30 Live 3-Digit Period Tracking Chain:** `{per_30}`"
    )
    st.markdown(
        f"📊 **Auto-Frequency Tracker (0-9 Exact Density):** `{freq_dict}`"
    )
...
if len(st.session_state.result_history) >= 1:
  ...
  if big_counts_30 >= 20:
    ...
    movement_desc = (
        "Reversal probability peak reached. Switching signal to Big."
    )
  elif is_dragon_5:
