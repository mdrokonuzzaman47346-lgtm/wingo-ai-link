...
if "ai_intelligence_reports" not in st.session_state:
    st.session_state.ai_intelligence_reports = []
...
with c1:
    st.markdown(
        ...
        " 10,000,000 MEGA DATA BASE: ONLINE<br>"
        "<small"
...
with c2:
    st.markdown(
        ...
        " HIGH-QUALITY AI CORE SERVER v12.1:<br>"
        "<small"
...
with col1:
    ...
    with b1:
        if st.button("🚀 ➕ Add Data to History", use_container_width=True):
            ...
            # Automatically record a complete report entry for AI Intelligence Report Center
            if "last_computed_prediction" in st.session_state:
                comp = st.session_state.last_computed_prediction
                report_entry = {
                    ...
                    "Actual Result": log_result,
                    "Win / Loss": bs_wl,
                    "BIG / SMALL Prediction": comp.get("prediction", "-"),
                    "BIG / SMALL Actual": actual_bs,
                    "RED / GREEN Prediction": comp.get("color_prediction", "-"),
                    "RED / GREEN Actual": actual_color,
                    "Confidence %": comp.get("confidence", "-"),
                    "Strategy Mode": comp.get("strategy_mode", "-"),
                    "Pattern Detected": comp.get("pattern", "-"),
                    "Movement Status": comp.get("movement_status", "-"),
                    "Target Numbers": comp.get("target_numbers", "-"),
                    "AI Status": "ONLINE",
                    "Session Type": comp.get("session_type", "-"),
                    "Important Analysis": comp.get("movement_desc", "-"),
                }
                st.session_state.ai_intelligence_reports.append(report_entry)
            st.rerun()
    with b2:
...
if len(st.session_state.result_history) >= 1 or (live_df is not None and not live_df.empty):
    ...
    detected_pattern_name = "Balanced Static Trend"
    if is_choppy_trap:
        omni_ai_weight = (
        ...
        detected_pattern_name = "Choppy/Trap Market"
        ...
        detected_pattern_name = "Triple Number"
        ...
        detected_pattern_name = "Double/Repeated Number"
    elif is_dragon_5:
        next_shot = last_real_size
        ...
        detected_pattern_name = "5-Round Deep Dragon"
    elif is_dragon_3:
        next_shot = last_real_size
        ...
        detected_pattern_name = "3-Round Dragon"
    elif is_zigzag_3:
        ...
        detected_pattern_name = "Zig-Zag (1-1)"
        ...
        detected_pattern_name = "Big Imbalance Reversal"
        ...
        detected_pattern_name = "Small Imbalance Reversal"
    elif is_step_121:
        next_shot = "SMALL" if last_real_size == "BIG" else "BIG"
        ...
        detected_pattern_name = "1-2-1 Step Pattern"
    elif is_mirror_6:
        next_shot = "SMALL" if last_real_size == "BIG" else "BIG"
        ...
        detected_pattern_name = "Symmetry Mirror"
        ...
        detected_pattern_name = "Double-Chain (2-2)"
    else:
        omni_ai_weight = (
            old_num + new_num + current_period_last_digit + diff + (diff % 3)
        ) % 2
    ...
    if len(st.session_state.history_records) >= 2:
        ...
        if all(wl == "L" for wl in recent_wl_logs):
            ...
            if len(st.session_state.history_records) >= 3 and st.session_state.history_records[-1]["bs_wl"] == "L" and st.session_state.history_records[-2]["bs_wl"] == "L":
                ...
                detected_pattern_name = "Fail-Safe Override"
    ...
    # Save current computed attributes in session state to be logged when result is added
    st.session_state.last_computed_prediction = {
        "prediction": next_shot,
        "color_prediction": predicted_color_code,
        "confidence": confidence_display,
        "strategy_mode": session_name,
        "pattern": detected_pattern_name,
        "movement_status": movement_mode_text,
...
# ====================================================
# 🧠 AI Intelligence Report Center (New Added Section)
# ====================================================
st.write("---")
st.markdown("### 🧠 AI Intelligence Report Center")

# Compute report statistics from recorded report entries
total_reports = len(st.session_state.ai_intelligence_reports)
total_wins = sum(
    1 for r in st.session_state.ai_intelligence_reports if r.get("Win / Loss") == "W"
)
total_losses = sum(
    1 for r in st.session_state.ai_intelligence_reports if r.get("Win / Loss") == "L"
)
win_rate = (
    (total_wins / total_reports * 100) if total_reports > 0 else 0.0
)

col_rep1, col_rep2, col_rep3, col_rep4 = st.columns(4)
with col_rep1:
    st.metric("📊 Total Predictions", total_reports)
with col_rep2:
    st.metric("✅ Total Win", total_wins)
with col_rep3:
    st.metric("❌ Total Loss", total_losses)
with col_rep4:
...
with col_dl:
    if total_reports > 0:
        df_report = pd.DataFrame(st.session_state.ai_intelligence_reports)
        output = BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            df_report.to_excel(writer, index=False, sheet_name="AI_Report")
        excel_data = output.getvalue()
        st.download_button(
            label="📥 Download AI Report",
            data=excel_data,
            file_name="AI_Intelligence_Report.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
    else:
        # Fallback or disabled state button if no records exist yet
        st.button("📥 Download AI Report", disabled=True, use_container_width=True)

with col_rs:
    if st.button("🗑 Reset AI Report", use_container_width=True):
        st.session_state.ai_intelligence_reports = []
        st.rerun()

st.markdown("---")
st.markdown("📌 Report Status")
st.markdown("● Recording Live Analysis...")
