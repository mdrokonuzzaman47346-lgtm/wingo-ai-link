...
if st.button("🚀 ACTIVATE OMNI-QUANTUM MATRIX SCAN"):
    ...
    if total_cases == 0 or total_cases > 0:
        big_count = sum(1 for num in matches if num >= 5) if total_cases > 0 else 0
        small_count = sum(1 for num in matches if num <= 4) if total_cases > 0 else 0
        
        # গেমের আদিম চরিত্রের ওপর ভিত্তি করে সরাসরি শক্তিশালী সিগন্যাল হিট করা
